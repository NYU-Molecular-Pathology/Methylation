...
generateQCreport <- function(runID = NULL) {
    ...
    if (!is.null(qcFileName)) {
        ...
        if (dir.exists(qcCache)) unlink(qcCache, recursive = TRUE)  # clear cache
        try(gb$uploadToRedcap(outQCpath, FALSE), silent = TRUE)
    } else{
...
check_csv_data <- function(sheetName = "samplesheet.csv") {
    ...
    if (sheetRunID != gb$runID) {
        ...
        if (stringr::str_detect(pattern = "new-template", gb$runID)) {
            message(csv_data$Project[1])
        } else {
            stopifnot(gb$runID == csv_data$Project[1])
        }
    }
