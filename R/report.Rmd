...
gb$familia <- out_class_family$`Methylation Family`[1]
gb$fscore <- out_class_family$`Class Score`[1]
gb$subfam <- out$`Methylation Subgroup`[1]
gb$subScore <- out$`Subgroup Score`[1]
...
```{r, label='MLH1 plot'}
gb$mlhP <- gb$pipeline(RGset2,getFig = F, getTable = T)
gb$theMlhTab <- drawPlotTab(gb$mlhP)
gb$theMlhTab
```
