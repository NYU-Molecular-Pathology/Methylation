...
h1.title {
    color: white;
    background-image: linear-gradient(to right, indigo, indigo, indigo, white, white);
    border-radius: 18px;
    padding: 5px;
    border-spacing: 5px;
}
...
<div id="wrapper"><div id="second">
<img src="https://raw.githubusercontent.com/NYU-Molecular-Pathology/Methylation/main/dockerScripts/logos/logos.png" width="200px"></div></div>

```{r global_options, include=FALSE, warning=FALSE, error=FALSE}
stopifnot(!is.null(params$rundata))
message("params$rundata")
message(paste0(capture.output(params$rundata), collapse="\n"))
...
```{r setup, cache.path="cache/", include=FALSE, cache=FALSE, error=FALSE}
...
body{margin-left: 30px!important;}
</style>
...
```{r 'Sample Info', class.source="bg-success", size="normalsize", error=FALSE}
gb <- globalenv(); assign("gb", gb)
dat <- params$rundata
sample = 1
gb$run_id = paste(dat$run_id)
gb$tech = paste(dat$tech)
gb$tech2 = paste(dat$tech2)
gb$bnumber = paste(dat$bnumber)
gb$mp_number = paste(dat$mp_number)
gb$sampleID = paste(dat$sampleID)
RGset = params$RGsetEpic
gb$RGset2 <- RGset <- RGset[,sample]
FFPE=NULL
...
gb$redcsv <- file.path(redfolder,redFile)

gb$is450k <- is450k <- RGset@annotation[[1]] == "IlluminaHumanMethylation450k"

gb$redcapDF <- data.frame(
    record_id = character(),
    b_number = character(),
...
if (is450k) {
	...
	# sex <- ifelse(mnp.v11b4::MNPgetSex(Mset)$predictedSex == "M", "male", "female")
		...
		gb$Mset <- mnp.v11b6::MNPpreprocessIllumina(RGset)
		...
		gb$Mset_ba <- mnp.v11b6::MNPbatchadjust(Mset, FFPE)
...
gb$sex <- sex
gb$sentrix <- sentrix <- colnames(RGset)
...
```{r 'Brain Classifier Family', size="normalsize", error=FALSE}
...
tryCatch(
    expr = {
        probs <- mnp.v11b6::MNPpredict(gb$Mset_ba[, 1], type = 'prob')
        probs_mcf <-
            mnp.v11b6::MNPpredict(gb$Mset_ba[, 1], type = 'prob', MCF = TRUE)
    },
    error = function(e) {
        ...
        probs <- mnp.v11b6::MNPpredict(gb$Mset[, 1], type = 'prob')
        probs_mcf <- mnp.v11b6::MNPpredict(gb$Mset[, 1], type = 'prob', MCF = TRUE)
    }
)
stopifnot(!is.null(probs))
stopifnot(!is.null(probs_mcf))
...
gb$out <- probs[oo[1:5]]
gb$out <- cbind(round(pmax(pmin(gb$out,1 - eps),eps),3),colnames(probs)[oo][1:5])

colnames(out_class_family) <- c("Class Score","Methylation Family")
colnames(gb$out) <- c("Subgroup Score","Methylation Subgroup")

gb$idx <- match(colnames(probs)[oo][1],mnp.v11b6::reflist[,2])
gb$out_class_family <- as.data.frame(out_class_family)
gb$out <- as.data.frame(gb$out)
gb$out_score <- as.numeric(paste0(gb$out$`Subgroup Score`[1]))

subVal_int <- NULL
if (is.null(gb$out_score)|is.na(gb$out_score)) {
    warning("out_score value (sub score) is NULL.  Assigning value of 0.00")
    gb$out_score <- 0.000
}
if (gb$out_score >= 0.500) {subVal_int <- "Positive"}
if (gb$out_score < 0.500 & gb$out_score >= 0.100) {subVal_int <- "Indeterminate"}
if (gb$out_score < 0.300) {subVal_int <- "Negative"}
if (is.null(subVal_int)) {
    warning("subscore value (subVal_int) is NULL")
    subVal_int <- "Indeterminate"
}

gb$out$Interpretation = c(subVal_int,"","","","")
fsco <- as.numeric(paste0(gb$out_class_family$`Class Score`[1]))
...
gb$out_class_family$Interpretation = c(famVal,"","","","")
...
gb$familia <- gsub(",","",gb$out_class_family$`Methylation Family`[1]) 
gb$fscore <- gb$out_class_family$`Class Score`[1]
gb$subfam <- gsub(",","",gb$out$`Methylation Subgroup`[1])
gb$subScore <- gb$out$`Subgroup Score`[1]
...
`r I(as.character(mnp.v11b6::reflist[gb$idx,7]))`
</p>
<hr class="solid">
...
```{r, label='TSNE Cluster', fig.height=8, fig.width=12, eval=FALSE}
...
a <- list(x = t1, y = t2, text = plTxt, xref = "x", yref = "y", size = 0.6, 
...
```{r, label='CNV Plot', fig.width = 12, fig.height=7, error=FALSE}
if (is450k==TRUE) {
...
```{r,label='MGMTPred', results='asis', out.width='50%', out.height='200px', plotly=TRUE, include=TRUE, fig.keep='all', fig.height=0.3, fig.width=1.5, error=FALSE}
#
...
```{r,label='MGMT tab', results='asis', fig.keep='all', out.width='50%', out.height='200px', plotly=TRUE, fig.height=1.5, fig.width=3.5, include=TRUE, self.contained=TRUE, error=FALSE}
#
if (is450k) {
...
<div>
```{r, label='MLH1 Functions', fig.align="center", fig.height=12, fig.width=12, dpi=40, error=FALSE}
gb$get.clinical.data = function(ratioSet, RGset){
...
gb$getPlotTable = function(clin.data){
...
gb$pipeline = function(RGset, getFig=F, getTable=F) {
...
```{r, label='MLH1 Prediction', fig.align="center", fig.height=8, fig.width=10, dpi=36, error=FALSE}
theMlhplot <- gb$pipeline(gb$RGset2, getFig = T, getTable = F) 
supM(theMlhplot)
```
</div>
...
```{r, label='MLH1 plot', error=FALSE}
gb <- globalenv(); assign("gb", gb)
mlhP <- gb$pipeline(gb$RGset2, getFig = F, getTable = T)
gb$theMlhTab <- gb$drawPlotTab(mlhP)
gb$theMlhTab
gb$mlhP <- mlhP

stopifnot(!is.null(gb$sampleID))
stopifnot(!is.null(gb$sentrix))
stopifnot(!is.null(ifelse(gb$is450k, yes = "450k", no = "EPIC")))
stopifnot(!is.null(tolower(gb$sex)))
stopifnot(!is.null(gb$familia))
stopifnot(!is.null(gb$fscore))
stopifnot(!is.null(gb$subfam))
stopifnot(!is.null(gb$subScore))
stopifnot(!is.null(gb$mgmtData$Status))
stopifnot(!is.null(gb$mlhP[[1]][1]))
stopifnot(!is.null(gb$mlhP[[1]][2]))
stopifnot(!is.null(gb$tech2))
stopifnot(!is.null(gb$tech))
stopifnot(!is.null(gb$run_id))
stopifnot(!is.null(gb$mp_number))
arrayType <- ifelse(gb$is450k, yes = "450k", no = "EPIC")
thesex<- paste0(tolower(gb$sex))

dfNewRed <- data.frame(
    record_id = gb$sampleID,
    b_number = gb$bnumber,
    barcode_and_row_column = gb$sentrix,
    array_type = arrayType,
    classifier_sex = thesex,
    classifier_score = gb$familia,
    classifier_value = gb$fscore,
    subgroup = gb$subfam,
    subgroup_score = gb$subScore,
    mgmt_status = gb$mgmtData$Status,
    mlh1_status = gb$mlhP[[1]][1],
    mlh1_pos_loci = gb$mlhP[[1]][2],
    second_tech = gb$tech2,
    primary_tech = gb$tech,
    run_number = gb$run_id,
    tm_number = gb$mp_number
)
stopifnot(!is.null(dfNewRed))
writeRedcap <- function(redcsv, dfNewRed, dfRedcap){
	redDF <- rbind(dfRedcap, dfNewRed)
	row.names(redDF) = NULL
	#write.table(redDF, file= redcsv, row.names=F, col.names=T, append=T)
	write.csv(x = redDF,file = redcsv, row.names = F, quote=F)
}
deskPath <- file.path("~", "Desktop", gb$run_id)
...
stopifnot(!is.null(colnames(dfRedcap)))
...
```{r, label='garbage collection', echo=FALSE, message=FALSE, error=FALSE, include=FALSE}
gc()
```
</div>
