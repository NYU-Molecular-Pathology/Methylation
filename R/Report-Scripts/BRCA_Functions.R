...
GetBRCA_data <- function(ratioSet, RGset, brcaProbes){
    ...
    for (bal in 1:nrow(brcaBetas)) {
        ...
        if (brcaBetas[bal, 1] < 0.09) {brcaBetas[bal, 1] = 0.00}
        if (brcaBetas[bal, 1] > 1.00) {brcaBetas[bal, 1] = 1.00}
        else (brcaBetas[bal, 1] = brcaBetas[bal, 1])}
    #Calculate detection of Pvalues using minfi method #c(MGMT_CGIDS,brcaProbes),]
...
renderBRCA_plot <- function(beta.matrix, clin.res, brca="BRCA1") {
    ...
    density <- gb$sanitizeDense(beta.matrix)
