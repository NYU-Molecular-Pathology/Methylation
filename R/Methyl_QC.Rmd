...
is_new <- stringr::str_detect(params$knitDir, pattern = "-new-template")
runID <- gb$CheckParamRunID(params, gb)
if (is_new) {
  runID <- basename(params$knitDir)
}
baseDir <- gb$CheckParamBaseDir(params, runID)
runPath <- gb$CheckParamKnitDir(params, runID)
is_new <- stringr::str_detect(runID, pattern = "-new-template")

runPath <- gb$CheckParamKnitDir(params, runID)
if (is_new) {
  runPath <- file.path(dirname(runPath), runID)
  baseDir <- file.path(dirname(baseDir), runID)
}
gb$runPath <- runPath
gb$SetKnitPath(runPath, baseDir)
# Read Sample Data -------------------------------------------------
sheetNamePath <- gb$GetSheetNamePath(params, baseDir)
if (!file.exists(sheetNamePath)) {
  sheetNamePath <- dir(path = params$knitDir, pattern = "samplesheet.csv", full.names = TRUE)
}
...
if (!file.exists(redCsv)) {
  gb$redCsv <- redCsv <- dir(params$knitDir, pattern = "_Redcap.csv", full.names = TRUE)[1]
}
# Set Run Data Values ----------------------------------------------
...
gb$close_page_and_exit <- function(page_num) {
    footer <- sprintf('<footer><p>Page %s of 8</p></footer></div>', page_num)
    knitr::knit_exit(append = footer)
}
if (!file.exists(gb$redCsv)) gb$close_page_and_exit(7)
mnpOutTb <- as.data.frame(read.csv(gb$redCsv, strip.white = T))
mnpOutTb <- unique(mnpOutTb)
if (!nrow(mnpOutTb) > 1) gb$close_page_and_exit(7)
...
gb <- globalenv(); assign("gb", gb)
xlsmSheet <- dir(gb$runPath, pattern = ".xlsm", full.names = TRUE)[1]
if (!file.exists(xlsmSheet)) {gb$close_page_and_exit(8)}
