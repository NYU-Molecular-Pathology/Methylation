...
if(is.null(params$reinstall)){params$reinstall=FALSE}
...
printParams <-function(varsNames){
  ...
  message("\nUse ", crayon::bgBlue('setVar("varNameString", varValue)'),
...
copyWorksheetFile <- function(runID=NULL, runYear=NULL) {
  ...
  if (dir.exists(mountLoc)) {
    ...
    if (file.exists(fileLoc)) {
      ...
      message("\nMake sure your path is correct, try print(copyWorksheetFile)")
    }
  } else{
...
setVar("runID", params$runID); setVar("ApiToken", params$token)
    methylPath <- setRunDir(runID); setwd(methylPath); sysPat <- paste0("cd ",methylPath)
    system(sysPat)
    varNm <- getDefaults(); varNm$ApiToken <- ApiToken; printParams(varNm)
    if(params$reinstall){install.or.load(instNew = T, rmpkg = T)}else{install.or.load(instNew = F, rmpkg = F)}
```

```{r sampleRunning, include=FALSE, cache=FALSE, progress=TRUE, verbose=TRUE, echo=TRUE }
    copyWorksheetFile(runID=params$runID) # copies the xlsm file
    ...
    moveSampleSheet(methDir) #copies outputs temp to desktop for QC.Rmd
    startRun(params$selectRDs)
```
