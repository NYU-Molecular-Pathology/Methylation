...
BioCinst <- function(pkg) {
    if (CheckReq(pkg)) {
        do.call(BiocManager::install, list(pkg, update = F, ask = F))
    }
    return(LibLoad(pkg))
}
CheckCran <- function(pkg) {
    if (CheckReq(pkg)) {
        do.call(install.packages, list(pkg, dependencies = T))
    }
    return(LibLoad(pkg))
}
...
plot_beta_values_distributions <-
    function(beta_values_matrix,
             specific_probes,
             geneName) {
        imgName <- paste(geneName, "probes_dist.png", sep = "_")
        beta_vals_vec <- as.vector(beta_values_matrix)
        probe_names <- rownames(beta_values_matrix)
        methylation_data <- data.frame(Beta_Value = beta_vals_vec,
                                       Probe_Name = probe_names)
        methylation_data_subset <-
            methylation_data[methylation_data$Probe_Name %in% specific_probes,]
        pltTtl <-
            paste("EPIC Methylation Beta Values Distributions for ",
                  geneName,
                  "Probes")
        plot2 <- ggplot(methylation_data_subset, aes(x = Beta_Value)) +
...
SaveBoxPlot <- function(thePlot, ggTable, tbl1, tbl2 = NULL){
...
PlotIslandProbes <- function(beta_values, anno, geneName = "BRCA1") {
    ...
    SaveBoxPlot(thePlot, ggTable, tbl1, tbl2)

    # Plot the beta values distributions for specific probes
    specific_probes <- rownames(island_probes)
