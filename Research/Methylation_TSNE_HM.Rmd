...
if (!requireNamespace("knitr", quietly = T)) install.packages("knitr", ask = F, dependencies = T, type = "binary")
library("knitr")
...
```{r,label='Project Info', child='params/about.Rmd', include=TRUE, cache=FALSE}
```

# Input Sample Sheet Information
```{r, label='Print Samplesheet', out.width='60%', echo=FALSE, message=FALSE, results='asis', cache=FALSE}
...
if (gb$genHeatMaps == T) gb$GetSuperHmData(gb, targets, RGSet)
```
