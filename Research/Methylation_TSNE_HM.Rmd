...
knitr::opts_chunk$set(echo = FALSE, error = FALSE, message = FALSE, warning = FALSE, cache = FALSE)
...
```{r ,label='Unsupervised_TSNE', opts.label='tsnechunk', fig.path=tsneOutU}
gb$MakeHideButton("unsupertsne")
targets1 <- gb$SubsetTargets(targets, gb$variable_to_filter)
gb$GenerateUnsuperTsne(targets1, betas, gb, shapeVariable = gb$col_shapes)
...
gb$LoopSupervisedPlots(targets, gb)
```
...
if (gb$genHeatMaps == T){
  gb$LoopPrintHeatMap(gb, unBetas = hmPlotData$unBetas, ha = hmPlotData$ha, geneNams = gb$addGenesHm, colSplt = 5)
}else{
...
```{r, label='segfile-generation', opts.label='cn_opts_1', include=FALSE}
if (gb$genCNchunk){
    cnData <- gb$grabClusterDat(gb$seg_clust_file, gb$segFile, targets, gb$idatPath)
    gb$loadHtmlTag()
}
```
...
<hr style="border:3px solid gray">
```{r, label='sample-cnv-generation', opts.label='gencnv', include=TRUE}
if (gb$genCNchunk) {
...
```{r, label='CNV-plots', opts.label='cn_opts_2', include=TRUE}
if (gb$genCNchunk){
    gb$SaveLoadCnvs(cnData[, 1:(ncol(cnData))], plotName = "All", plotTitle="All")
  gb$GenCNVdataGroupSave(cnData, targets, gb$col_samGrp)
} else{
  cat("No CNV analysis output")
}
if (gb$genPathChunk == FALSE) {
