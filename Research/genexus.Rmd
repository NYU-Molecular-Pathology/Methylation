...
params:
  outDir: "/Volumes/molecular/Molecular/Validation/Genexus/Results/2022/new_run"
---
...
dropDots <- function(qcTab){
  colnames(qcTab)<- stringr::str_replace_all(colnames(qcTab),"\\.", " ")
  colnames(qcTab)<- stringr::str_replace_all(colnames(qcTab),"  ", " ")
  colnames(qcTab)<- stringr::str_replace_all(colnames(qcTab),"_", " ")
  return(qcTab)
}
...
modColNames <- function(kwd, modData, selected){
...
makeCompare <- function(sam, snvOut) {
  ...
  tryCatch(
    expr = {
      nexus <- modColNames(kwd = "Genexus", nexus, genxCol)
    },
    error = function(e) {
  ...
  philips <- modColNames(kwd = "PACT", snvOut, philCol)
