...
# To Knit RMD, copy to same directory:"_MethylMatch.xlsx","-QC.tsv","-SampleSheet.csv", and all ".FACETS.cnv.plot.pdf" files
outDir <- file.path(workDir, paste0(pactName,"_consensus"))
opts_knit$set(root.dir = outDir)
...
pkgs <- c("animation","tidyverse","knitr","kableExtra","stringr","readr","ggplot2","data.table","plotly","DT","pdftools", "htmltools")
...
pngDir <- "/Volumes/molecular/Molecular/MethylationClassifier/CNV_PNG" # methylation PNG creator
...
if(!file.exists(descrip)){
  warning(paste("You must create a description file:", descrip, "\nFile must be in same directory as Rmd:", getwd()))
}
...
message("methData file below:"); print(methData[,c(1:8,11)])
methSams <- methData$Test_Number
message("methSams listed below:"); cat(methSams)
...
message("Your samples:"); print(samples)
sampleRows <- c(1:length(samples)) # if you have half part change to 1:8 or 32 etc
message("Number of sample rows:"); print(sampleRows)
qcData <- read.delim(file=qcTsv)
row.names(qcData)<-qcData$Row
message("qcData:"); print(qcData[,2:4])
snvDt <- read.csv(descrip)
message("Your description file heading:"); print(head(snvDt))
```
...
```{r, label='PNG Output', echo=FALSE, message=TRUE, warning=FALSE, results='asis', error=FALSE, fig.keep='all', include=TRUE, fig.align="center", fig.cap="CNV plot", fig.show="asis", out.width = '100%', include=FALSE, fig.width=15}
library("kableExtra")
...
message("qcHeader tab headings used:"); print(qcHeader)
...
makeDT <- function(tabNam, objDat, pdfFi=NULL, rdNumb=NULL, sam=NULL){
  ...
  dtOpts <- list(scrollX = T, scrollY=T, info = F, autoWidth = T, pageLength = 15,
  ...
  if(!is.null(objDat)){
    if(tabNam=="QC"){
      ...
      cat("\n\n")
        ...
        dtTab <- htmltools::tagList(DT::datatable(objDat, rownames = F, options=dtOpts, height = "120%"))
      print(dtTab)
        cat("\n\n")
    }
  }else{
...
convert.plots <- function(samList, pngOutDir) {
  ...
  ngsOrder = which(sapply(tumors$Specimen_ID, grepl, pdfList), arr.ind = T)[, "row"] # re-order matching rows
  invisible(lapply(X=1:length(ngsOrder), FUN=function(X){
    pngName <- paste0(tumors$Test_Number[X], ".png") # output file
    pdfFile <- pdfList[ngsOrder[X]] # matching pdf FACET file
    suppressWarnings(pdftools::pdf_convert(pdfFile, filenames = file.path("cnvpng", pngName), dpi=200)
      )}))
  pdfDir <- file.path(getwd(),"FACETpdfs")
...
makeAbTab <- function(sam){
  ...
  if(file.exists(snvCsv)){
    ...
    snvOut <- snvOut[,c(1:9,14)]
    colOrder <- c("HGNC_gene","AberrationType","Coordinate",
                  "tumor_freq","normal_freq","tumor_dp","normal_dp",
                  "THERAPY_AVAILABILITY","Canonical_HGVS_Protein","SomaticStatus")
    ...
    cat("\n\nNote: These are all the Indels on Philips with tumor_freq above or equal to 5%\n\n")
    cat("\n\n")
  }
  if(file.exists(samCsv)){
...
pngOutDir <- file.path(outDir,"cnvpng")
message("Checking if cnvpng folder exists...")
if(!dir.exists(pngOutDir)){convert.plots(samList, pngOutDir)}
```
  
```{r, label='Table Output', echo=FALSE, warning=FALSE, results='asis', error=FALSE, fig.keep='all', include=TRUE, fig.align="center", fig.cap="CNV plot", fig.show="asis", out.width = '100%', fig.width=15}  
