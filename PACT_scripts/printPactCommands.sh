...
msg_step 1 "#ffffba" "After Demux finishes, check the output folder for QC file by pasting the link below in a web browser to open in a SFTP client like CyberDuck to view:"
...
msg_step 9 "#ffffba" "Check Philips IntelliSpace to make sure file uploads are not pending review or missing and wait for the pipeline completion email from Nextflow"
echo "$BOX2"
...
msg_step 2 "#d9d2e9" "Go to the run production directory and edit permissions"
msg_code "cd ${molecDir}${runID} && chmod -R g+rwx ${productionDir}/NGS607/${runID}/output/"
...
msg_step 4 "#d9d2e9" "Change permissions for group access to the new files"
...
msg_step 4 "#baffc9" "Next, go to /MOLECULAR LAB ONLY/NYU PACT Patient Data/ on the Z-drive and copy the remaining clinical output files"
...
msg_step 1 "#bae1ff" "Create a new directory for your PACT consensus locally"
msg_code "mkdir \"${consensusDir}${pactRun}_consensus\" && cd \"${consensusDir}${pactRun}_consensus\""
msg_step 2 "#bae1ff" "Download the template RMD file from GitHub to that folder"
msg_code "curl -o ${pactRun}_consensus.Rmd -L ${pactGithub}/PACT_consensus.Rmd -s"
msg_step 3 "#bae1ff" "To this directory, copy all the .cnv.plot.pdf facets files from the Z-drive and the MethylMatch.xlsx file from the Desktop"
...
msg_code "open ${consensusDir}${pactRun}_consensus/${pactRun}-Somatic_Variants.html"
