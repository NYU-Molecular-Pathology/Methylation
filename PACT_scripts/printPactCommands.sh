...
msg_code "make update"
msg_code "make submit"
echo "$BOX2"
...
msg_code "cd $DEMUXDIR"
msg_code "make passed"
msg_code "make uploads"
...
msg_code "python ${reprtDir}snp_overlap.py -o ${molecDir}${runID}/output"
msg_code "python ${reprtDir}generate_html_report.py -o ${molecDir}${runID} -p ${pactRun} -r ${runID}"
...
msg_code "mkdir -p \"${zdrive}/NGS607/${currYear}/${runID}/output/alignments\""
msg_code "mkdir -p \"/mnt/${kerbero}${outputDir}${currYear}/${pactRun}\""
msg_code "chmod -R g+rwx \"/mnt/${kerbero}${outputDir}${currYear}/${pactRun}\" "
#msg_code "mkdir -p ${rsyncDir}/"
msg_code "chmod -R g+rwx /gpfs/home/${kerbero}/molecpathlab/production/NGS607/${runID}/output"
msg_step 3 "#baffc9" "Once created, copy files to /MOLECULAR/NGS607/"
msg_white "<b>Copy Alignments:</b>"
msg_code "rsync -vrthP ${rsyncDir}/alignments/${FG_GRN}deduplicated${normal} ${zdrive}/NGS607/${currYear}/${runID}/output/${FG_GRN}alignments/${normal}"
msg_white "<b>Copy Annotations:</b>"
msg_code "rsync -vrthP ${rsyncDir}/${FG_GRN}annotations${normal} ${zdrive}/NGS607/${currYear}/${runID}/output/${FG_GRN}annotations/${normal}"
msg_white "<b>Copy Clinical:</b>"
msg_code "rsync -vrthP ${rsyncDir}/${FG_GRN}clinical${normal} ${zdrive}/NGS607/${currYear}/${runID}/output/"
msg_white "<b>Copy .tsv files:</b>"
msg_code "rsync -vrthP ${rsyncDir}/${FG_GRN}*.tsv${normal} ${zdrive}/NGS607/${currYear}/${runID}/"
msg_step 4 "#baffc9" "Next, copy files to /MOLECULAR LAB ONLY/NYU PACT Patient Data/"
#msg_code "mkdir \"/mnt/${kerbero}${outputDir}${currYear}/${pactRun}/\""
#msg_code "chmod g+rwx \"/mnt/${kerbero}${outputDir}${currYear}/${pactRun}/\""
msg_code "cd \"/mnt/${kerbero}${outputDir}${currYear}/${pactRun}/\""
msg_code "rsync -vrthP ${rsyncDir}/${FG_GRN}clinical/${normal} ./"
...
msg_code "rsync -vrthP ${productionDir}/NGS607/${runID}/${pactRun}-QC.tsv ${zdrive}/REDCap/cnv_facets/${pactRun}/"
...
msg_code "rsync -vrthP ${productionDir}/NGS607/${runID}/output/cnv/FACETS/*.pdf ${zdrive}/REDCap/cnv_facets/${pactRun}/"
msg_step 8 "#bae1ff" "Once you have completely filled the consensus description file, knit the Rmd in Rstudio and view it"
msg_code "Rscript --verbose -e \"rmarkdown::render('${pactRun}_consensus.Rmd', params=list(pactName='${pactRun}',userName='Jonathan'))\""
msg_step 9 "#bae1ff" "After the concensus html file is created, copy to the output folder"
