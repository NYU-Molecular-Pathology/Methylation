...
msg_code "/gpfs/home/${kerbero}/molecpathlab/scripts/bash_helpers/start_demux.sh ${runID}"
...
msg_step 2 "#ffffba" "Return to the Demux directory, execute the pass and upload *make* commands to upload to Philips, then deploy the in-house pipeline"
msg_code "ssh ${kerbero}@bigpurple.nyumc.org"
msg_code "/gpfs/home/${kerbero}/molecpathlab/scripts/bash_helpers/start_pipeline.sh ${runID}"
...
msg_step 4 "#ffffba" "ssh pgm@pgmlcdcpvm01.nyumc.org to the isg-uploads folder to ensure the files are accessible and check the pgm log to verify uploading to Philips every 30 min"
...
msg_step 2 "#baffc9" "ssh to the data mover node, ensure the the molecular drive is mounted, and create output directories in /MOLECULAR/NGS607/"
