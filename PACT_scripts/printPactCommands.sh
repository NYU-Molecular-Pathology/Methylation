...
msg_note "Reminder:" "Check Philips IntelliSpace to make sure file uploads are not pending review or missing and wait for the pipeline completion email from Nextflow"
echo "$BOX2"
...
msg_code "cd /gpfs/home/${kerbero}/miniconda3/bin/ && ./conda init"
...
msg_code "pip install -r /gpfs/home/${kerbero}/molecpathlab/development/NGS_QC_xf/requirements.txt"
msg_step 5 "#d9d2e9" "Go to the QC python directory and generate the QC file"
msg_code "conda activate && cd /gpfs/home/${kerbero}/molecpathlab/development/NGS_QC_xf/ && python xf_pactqc.py -rdir ${molecDir}${runID}/output -pactid ${pactRun} && conda deactivate"
echo "$BOX2"
...
msg_step 5 "#baffc9" "Email the PACT team once the QC files are copied to notify them the following"
