...
up <- function(){update.packages(repos='http://cran.rstudio.com/', ask=F, checkBuilt=T)}
...
gh.inst <- function(pkNam, ghLnk,...){
  ...
  gh.opt=list(ghLnk, dependencies=T, verbose=T, upgrade="always",...)
  tryCatch(
    expr={if(rq(pkNam)){
      params <- c(gh.opt,list(type="both"))
...
srcInst <- function(pkg,...){
  message("Checking ", pkg, "...")
  downLoc = file.path("~/Desktop",basename(pkg))
  message("Downloading to:\n",downLoc)
  download.file(url=pkg,destfile=downLoc)
  tryCatch(
    expr={
      install.packages(downLoc, repos=NULL,dependencies=T,verbose=T, type="source")
      },
    error=function(cond){
    ...
    finally={message(pkg," ...load successful")}
  )
}
...
fixNeeds <- function (){
  ...
  write(0, file=sysfile); options(needs.promptUser=FALSE); needs:::autoload(TRUE)
}
...
checkNeeds <- function(){
  tryCatch(
    ...
    warning=function(cond){
      ...
      message("needs ...install ", rq(needs))
    }
  )
}
...
loadPacks <- function(ezLibs=NULL, ghPk=NULL, bioPks=NULL) {
  pkgs <- c('devtools', 'remotes', 'sjmisc', 'stringi', 'digest', 'RCurl', 'rlang', 'parallel', 'grid', 'gridExtra','knitr', 'kableExtra','ggplot2', 'plotly', 'ggfortify', 'ggrepel', 'gplots', 'fastmatch','pals', 'Polychrome','qdapTools','beepr','xtable','pander','grDevices','graphics','stats', 'utils','magick')
  if(is.null(ezLibs)){
  ...
  if(is.null(bioPks)){
    bioPks <- c('HDF5Array','rngtools','bumphunter','minfi','lumi', 'ade4', 'methylumi',
                'randomForest', 'glmnet',
  ...
  tryCatch(
    expr={setOptions(); fixProf()
      ...
      if(!require("BiocManager")){BiocManager::install(version="3.10",update=T, ask=F, type="source")}
      ...
      # Loads libraries
      ready <- ezLibs %in% names(sessionInfo()[["otherPkgs"]])
      if(any(!ready)){lapply(ezLibs[!ready], require, character.only=TRUE)}
    },
    error= function(cond){
      message("You encountered the following error during install:\n", cond,"\n")
...
#' @return
#' @export
setRunType <- function(isMC=T){

  sexEst="https://github.com/jungch/sest/raw/master/sest.tar"
  mgmtLn="https://github.com/badozor/mgmtstp27/raw/master/trunk/Rpackage/mgmtstp27_0.6-2.tar.gz"
  if (isMC){
    cbio="/Volumes/CBioinformatics/"; zdriv="/Volumes/molecular/Molecular"; wmm ="You do not have this path mounted:\n"
    if(!dir.exists(cbio)){warning(wmm,cbio)}; if(!dir.exists(zdriv)){warning(wmm,zdriv)}
    ...
    if (rq("mnp.v11b6")){cat(ms1)}
    if (!rq("mnp.v11b6")){cat(ms2)}
    message("\nIf any compiler errors occur run fixCompiles()")
    #file.path(system.file('data', package = "mnp.v11b6"),'rfpred.v11b6.rds')
  }else{loadPacks()}
}
message("Run setRunType() to get started")
