...
require("needs")
library(knitr)
knitr::opts_knit$set(progress = TRUE, verbose = FALSE, comment='')
knitr::opts_chunk$set(error = FALSE, fig.keep = 'none', echo = FALSE, message = FALSE, warning = FALSE)
knitr::opts_knit$set(progress=TRUE, verbose=FALSE, fig.align="left", self.contained=TRUE)
knitr::opts_knit$set(progress = TRUE, verbose = FALSE)
knitr::opts_chunk$set(results = 'hide', child=NULL)
```

```{r setup, cache.path="cache/", include=FALSE, cache=TRUE}
knitr::opts_chunk$set(cache.extra = list(R.version, sessionInfo(), format(Sys.Date(), '%Y-%m')))
knitr::opts_chunk$set(self.contained=TRUE)
knitr::opts_chunk$set(error = FALSE, fig.keep = 'none', echo = FALSE, message = FALSE, warning = FALSE)
knitr::opts_knit$set(progress=TRUE, verbose=FALSE)
knitr::opts_current$set(progress=TRUE, verbose=FALSE, fig.align="left", self.contained=TRUE, error = FALSE, fig.keep = 'none', echo = FALSE, message = FALSE, warning = FALSE)
options(scipen = 5)
require(compiler)
...
suppressWarnings(library(verbose=F,warn.conflicts = F, quietly = T, package=ggplot2))
...
knitr::knit_hooks$set(time_it = local({
  ...
  function(before, options) {
    if (before) {
      ...
      message("Time for this chunk to run: ", round(res, 2), " secs")
    }
  }
}))
knitr::opts_knit$set(progress=TRUE, verbose=FALSE)
...
knitr::opts_knit$set(progress=TRUE, verbose=FALSE)
knitr::opts_chunk$set(comment = NA)
#, class.source="bg-success", size="normalsize"
sample = 1
run_id = paste(run_id)
tech = paste(tech)
tech2 = paste(tech2)
bnumber = paste(bnumber)
mp_number = paste(mp_number)
...
outPath <- file.path(getwd(),run_id)

outFiName <- paste0(run_id,"_Redcap.csv")
redcsv <- file.path(outPath,run_id,outFiName)
redfolder <- file.path(outPath, run_id)
...
```{r 'Brain Classifier Family', size="normalsize", time_it=TRUE}
knitr::opts_current$set(progress=TRUE, verbose=FALSE, fig.align="left", self.contained=TRUE, error = FALSE, fig.keep = 'none', echo = FALSE, message = FALSE, warning = FALSE)
...
xtraCss3="border-radius:0px;border-width:1px;border-style:solid;border-color:rgb(192,192,192);"
xtraCss4="border-radius:0px;border-width:3px;border-style:solid;border-color:rgb(16,28,105);"
...
out %>% 
	...
	kableExtra::column_spec(column=c(1,2),extra_css=xtraCss3) %>%
	...
	kableExtra::row_spec(row=1,extra_css=xtraCss4)
...
`r I(as.character(reflist[idx,7]))`
</p>
<hr class="solid">
...
```{r, label='TSNE Cluster', fig.height=8, fig.width=12,time_it=TRUE}
...
```{r, label='CNV Plot', fig.width = 12, fig.height=7,time_it=TRUE}
if (is450k==TRUE) {
    library(verbose=F,warn.conflicts = F, quietly = T, package="mnp.v11b4")
...
```{r,label='MGMTPred', results='asis', out.width='50%', out.height='200px', plotly=TRUE,time_it=TRUE}
#include=TRUE,fig.keep='all', fig.height=0.3, fig.width=1.5, 
...
```{r,label='MGMT tab', results='asis', fig.keep='all', out.width='50%', out.height='200px', plotly=TRUE,time_it=TRUE}
#fig.height=1.5, fig.width=3.5, include=TRUE, self.contained=TRUE, 
if (is450k) {
...
<div>
```{r, label='MLH1 Functions'}
#, fig.align="center", fig.height=12, fig.width=12, dpi=40
...
```{r, label='MLH1 Prediction', fig.align="center", fig.height=8, fig.width=10, dpi=36,time_it=TRUE}
knitr::opts_knit$set(progress=TRUE, verbose=FALSE)
knitr::opts_current$set(progress=TRUE, verbose=FALSE)
...
```{r, label='REDCapWrite', include=FALSE}
dfNewRed <- data.frame(
    record_id = sampleID,
    b_number = bnumber,
...
outFiName <- paste0(run_id,"_Redcap.csv")
redcsv <- file.path(outPath,outFiName)
