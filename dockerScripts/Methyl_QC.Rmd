...
pltNames <- c(
    ...
    "Biological Positive Control QC - Overall Amplification Performance (*SBPC > 11* is Passing)",
...
dat.hc <- get.hc.dat()
plotHC <- swm(plotParams(totNum, dParam = GetNewDt(dat.hc, cutoff = 12.75),
                         xincept = 12.75, yincept = 0))
...
runName <- gb$runID #paste(targets$RunID[1])
if (is.null(runName)) {runName = paste0(basename(getwd()))}
redCsv <- file.path(fs::path_home(),"Desktop",runName, paste0(runName,"_Redcap.csv"))
if(!file.exists(redCsv)){knitr::knit_exit()}
```

<h2>`r I(as.character(summarName))`</h2>

<div style="align:left;margin-right:auto;margin-left:0px;font-size:10pt">
```{r label="Summary Table", fig.align="left", message=FALSE, echo=FALSE, warning=FALSE}
library("kableExtra")
library("dplyr")
mnpOutTb <- read.csv(gb$redCsv, strip.white = T)
...
tableSum <- mnpOutTb %>% 
    ...
    dplyr::select(
        record_id, b_number, tm_number,
        classifier_score, classifier_value, 
        subgroup, subgroup_score, mgmt_status
        ) %>%
    kable(format = "html", kgh, align = "c", col.names = tableHeader) %>% 
...
<p class="comment" style="width:70%;">
<ul>
...
<p class="mnpnote"></p><p class="qcnote"></p>

<p class="comment">
```{r Summary_notes, echo=FALSE, fig.align="center", fig.width=18, fig.height=12, message=FALSE, warning=FALSE, results="asis"}
...
if (file.exists(worksheetNam) & file.exists(redCsv)) {
    noteData <- as.data.frame(readxl::read_excel(
        worksheetNam,
        sheet = 2,
        range = "M1:M7",
        col_types = c("text")
    ...
    noteData <- as.data.frame(noteData[!is.na(noteData),])
    noteData <- as.data.frame(noteData[noteData != 0, 1])
