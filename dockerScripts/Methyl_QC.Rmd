...
fixerrors <- gb$GetFixedDf(sheetNamePath, runPath)
totNum <- gb$GetTotalPairs(fixerrors)
targets <- GrabTargetsDf(runPath, fixerrors)
redCsv <- gb$CheckRedCsv(runID)
# Set Run Data Values ----------------------------------------------
...
---
title: "`r titleName`"
subtitle: "Clinical Methylation Run"
...
# MethylAid Summarize ------------------------------------------
sdata <- MethylAid::summarize(fixerrors, verbose = T, batchSize = gb$totNum, force = T, BPPARAM = mcParam)
## Get MU Data & Colors ----------------------------------------
...
## MU Plot --------------------------------------------------------------------------------
dat.mu <- swm(gb$GetNewDt(data_rotated))
...
<div style="align:left;margin-right:auto;margin-left:0px;font-size:10pt;width:950px">
...
failedSams <- GetFailedSams(mnpOutTb)
...
noteData <- GetNotesData(xlsmSheet)
