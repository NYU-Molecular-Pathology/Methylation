...
pk.in <- function(pkg){install.packages(pkg,dependencies=T,verbose=T,ask=F, type="binary")}
...
cranInstall <- function(pkg) {
  tryCatch(
    ...
    error = function(cond) {
      ...
      install.packages(pkg,dependencies=T,verbose=T,ask=F,type="source")
      ld(pkg)
      },
...
setRunType <- function(){
  mv11 = "The mnp.v11b6 package is missing to install or update use the function:\n install.or.load(instNew=T)"
  ...
  loadPacks()
    ...
    if (rq("rmarkdown")){gh.in('rstudio/rmarkdown')}
...
getSetvars <- function() {
  cbVol = "/Volumes/CBioinformatics"; moVol = "/Volumes/molecular"
...
defineParams <-
  function(mnp.pk.loc = NULL, ApiToken = NULL, methDir = NULL, clinDrv = NULL, rschOut = NULL, clinOut = NULL,
...
setDirectory <- function(foldr) {
  ...
  if (dir.exists(foldr)) {system(bsDir);setwd(foldr)} else{warning(mm2)}
}

listMolecularSheets <- function(returnAll=F) {
  ...
  cat(crayon::black$bgYellow("Newest Run Found:", crayon::red$bold(newestRun)))
  return(newestRun)
}
...
launchEmailNotify <-function(runID){
  ...
  com <- ifelse(isMC==T, "sample_qc", "sample_qc_research") # research or clinical notification
...
readSheetWrite <- function(sampleNumb= NULL, runID = NULL) {
 ...
 if (is.null(df$MP_number)){warning(ww2); df$MP_number <- "none"}
    ...
    } else{cat(crayon::white$bgGreen("samplesheet.csv already exists! Delete to create new csv if needed.\n"))}
}
...
checkPackages <- function(pathtoFile) {
  thisHist=file.path(getwd(),".Rhistory"); print(pathtoFile)
  if (!file.exists(pathtoFile)){warning("You must re-map path to /Volumes/CBioinformatics")}
  if (file.exists(thisHist)){file.remove(thisHist);Sys.setenv(R_ENABLE_JIT=T)}
...
install.or.load <- function(pathtoFile=NULL, instNew=F, rmpkg=F) {
  pathtoFile<-ckNull(pathtoFile, gb$mnp.pk.loc, deparse(substitute(pathtoFile,env=.GlobalEnv)))
...
# FUN: Copies Reports to Z drive
copy2outFolder <-
  function(clinDrv = NULL, runID, runYear = NULL) {
    if (is.null(runYear)) {runYear = paste0(format(Sys.Date(), "%Y"))}
...
makeReports.v11b6 <-
  function(runPath = NULL, sheetName = NULL, selectSams = NULL, genCn = F, skipQC = F,email = T, cpReport = F,redcapUp = T) {
    dsh <- "-----------"
    ...
    if(redcapUp==T){file.list <- list.files(path = gb$workDir, "*.html", full.names = T); uploadToRedcap(file.list)
    }
...
startRun <- function(selectRDs=NULL){
  ...
  if(stringr::str_detect(gb$runID, "MGDM")){
    #Clinical Run
