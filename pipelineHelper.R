...
pk.in <- function(pkg){install.packages(pkg,dependencies=T,ask=F, type="binary")}
# FUN: Installs package from GitHub
...
cranInstall <- function(pkg) {
  tryCatch(
    ...
    error = function(cond) {
      ...
      install.packages(pkg,dependencies=T,ask=F,type="source")
      ld(pkg)
      },
...
# FUN: Installs the mgmt classifier package if missing
...
checkQCpkg <- function(){
  methylQCpacks <- c(
    "kableExtra","magick","webshot","plyr","beepr","ggplot2","knitr","reshape2","data.table","DT","plotly",
    "MethylAid","minfi","scales","IlluminaHumanMethylation450kmanifest",
    "IlluminaHumanMethylationEPICmanifest","IlluminaHumanMethylationEPICanno.ilm10b4.hg19","Biobase",
    "RColorBrewer","limma","ggfortify","Rtsne","qdapTools","gplots","readxl","stringr","ggrepel","Polychrome",
    "tinytex","gridExtra","rmarkdown","tinytex", "BiocParallel", "grid"
    )
...
defineParams <- function(mnp.pk.loc = NULL, ApiToken = NULL, methDir = NULL, clinDrv = NULL, rschOut = NULL, clinOut = NULL,
...
# FUN: Check Packages are Installed & parallel called by install.or.load()
